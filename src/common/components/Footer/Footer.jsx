    ...
    const fetchData = async () => {
      try {
        const responseData = await sendRequest(
          `https://api.countapi.xyz/hit/http://localhost:3000/${process.env.REACT_APP_COUNTAPI_KEY}`
        );
  ...
  return (
    <div id="footer">
      ...
      <Stack className="copyright" direction="horizontal" gap={5}>
        <p>
          {' '}
          &copy; 2024 - {new Date().getFullYear()} Yashodip Beldar All rights
          reserved.
        </p>
        <p className="ms-auto">Made with 💖 by Yashodip Beldar!</p>
      </Stack>
    </div>
  );
};
